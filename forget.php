...
<body class="w3-content" style="max-width:1200px;">
        ...
        <div class="w3-display-container w3-container" style="border: #64F4AA 1px solid;">
            <form name="form1" action="" method="POST" id="form1">
                <table align="center" style="padding: 20px;">
                    ...
                    　 <tr>
                        <td>
                            密碼 : <input type="password" name="pw" style="font-size:15px" placeholder="*請輸入新密碼(6~12字元)" maxlength="12" minlength="6" required>
                        </td>
                        <td>
                    ...
                    <tr>
                        <td>
                            手機 : <input type="text" name="tele" style="font-size:15px" placeholder="*請輸入行動電話" required maxlength="10" onkeyup="sendRequest()">
                        </td>
                        <td>
                    ...
                    <tr>
                        <td>
                            信箱 : <input type="text" name="mail" style="font-size:15px" placeholder="*請輸入Email"  require onkeyup="sendRequest()">
                        </td>
                        <td>
 ...
 <script>
      ...
      xhttp.onreadystatechange = function() {
         if (this.readyState == 4 && this.status == 200) {
            if (this.responseText=='1') 
                {
                    document.getElementById('show_msg').innerHTML = '帳號存在';
                    $check = 1;
                }            
            else 
                {
                    document.getElementById('show_msg').innerHTML = '此帳號不存在或電話、信箱錯誤!';
                    $check = 0;
                }    
          
         }
      };
      var url='check_forget.php?id_number=' + document.form1.id_number.value +'&pw=' + document.form1.pw.value +'&phone=' + document.form1.tele.value +'&mail=' + document.form1.mail.value +'&timeStamp='+new Date().getTime();
   ...
   <script>
    ...
    if($check == 0)
        alert('修改失敗，請檢查帳號，信箱，電話是否正確');
    else
        {
            alert('修改成功');
            var xhttp = new XMLHttpRequest();
            var url='action_forget.php?id_number=' + document.form1.id_number.value + '&pw=' + document.form1.pw.value + '&phone=' + document.form1.tele.value+ '&mail=' + document.form1.mail.value+'&timeStamp='+new Date().getTime();
            xhttp.open('GET',url,true);//建立XMLHttpRequest連線要求
            xhttp.send();
        }
    }
